...
let rec read_components : exp -> typ -> t -> t
=fun e ty cur_comps -> begin
  ...
  | T_option ty' -> begin
      ...
      append_approach (read_components e' ty' cur_comps) option_app
    end
  | T_pair (ty1', ty2') -> begin
      let e1' = Vlang.create_exp_uni_op_car e ty in
      let cur_comps' = read_components e1' ty1' cur_comps in
      let e2' = Vlang.create_exp_uni_op_cdr e ty in
      let cur_comps'' = read_components e2' ty2' cur_comps' in
      cur_comps''
    end
  | T_or (ty1', ty2') -> begin
      let e1' = Vlang.create_exp_uni_op_un_left e ty in
      let cur_comps' = append_approach (read_components e1' ty1' cur_comps) or_left_app in
      let e2' = Vlang.create_exp_uni_op_un_right e ty in
      let cur_comps'' = append_approach (read_components e2' ty2' cur_comps') or_right_app in
      cur_comps''
    end
  | T_map (_, ty2') -> begin
      match ty2'.d with
