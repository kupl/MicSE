...
(* We separate tezos operation into two parts, 
    one is explicit operation that given by tezos client,
    the other is implicit operation generated by michelson code execution.
  Every successful execution of explicit operation updates fixed-blockchain of symbolic-state,
  but the successful execution of implicit operation only updates dynamic-blockchain.

  ++ Internal operations are the basic unit of operations that compose explicit and implicit operations.
    - internal operation looks & updates dynamic-chain only.
*)
and operation =
  (* explicit *)
  | TO_explicit_create_contract of (mich_v cc * (mich_v cc * mich_v cc * mich_v cc) * (mich_t cc * mich_t cc * mich_i cc)) (* sender-addr * (key_hash option * mutez * storage-val) * ('param-ty * 'storage-ty * code) *)
  | TO_explicit_transfer_tokens of (mich_v cc * mich_v cc * mich_v cc * mich_v cc) (* sender-addr * param-val * mutez * contract *)
  | TO_explicit_set_delegate of (mich_v cc * mich_v cc) (* sender-addr * key_hash option *)
  (* implicit *)
  | TO_implicit_create_contract of (mich_v cc * (mich_v cc * mich_v cc * mich_v cc) * (mich_t cc * mich_t cc * mich_i cc)) (* sender-addr * (key_hash option * mutez * storage-val) * ('param-ty * 'storage-ty * code) *)
  | TO_implicit_transfer_tokens of (mich_v cc * mich_v cc * mich_v cc * mich_v cc) (* sender-addr * param-val * mutez * contract *)
  | TO_implicit_set_delegate of (mich_v cc * mich_v cc) (* sender-addr * key_hash option *)
  (* internal *)
  | TO_internal_balance_sub of (mich_v cc * mich_v cc) (* target-addr * mutez(amount) *)
  | TO_internal_balance_add of (mich_v cc * mich_v cc) (* target-addr * mutez(amount) *)
    (* WARNING: "TO_internal_balance_add" does not put any terminated sym-state (i.e., it forces to put no-overflow constraint) 
...
val typ_of_val_i : mich_v -> mich_t cc
